#!/usr/bin/env python3
"""openpilot 배포 ONNX를 nncase가 읽을 수 있는 형태로 정규화한다.

openpilot v0.9.x 모델은 fp16 가중치/입출력으로 배포되는데 nncase 2.11의 PTQ
경로는 fp32 그래프를 요구한다. 나머지 옵션(중복 opset 병합, 빈 노드 이름
채우기, Reshape allowzero 제거)은 importer가 그대로 넘기지 못하는 표현만
정리하며 값은 바꾸지 않는다.
"""
