...
- 이 값은 조향 토크와 CAN 송신에 직접 영향을 준다. 반드시 정차 상태에서 수정한다.
...
| `max_lateral_accel` | 3.0 | m/s^2 / 0.5~5 | 속도별 최대 목표 곡률을 횡가속도 기준으로 제한한다. |
| `max_curvature` | 0.2 | 1/m / 0.01~0.5 | 최종 목표 곡률의 절대 상한이다. 0.2 1/m는 반경 약 5m에 해당한다. |
