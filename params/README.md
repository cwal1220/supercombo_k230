...
| `torque_low_speed_gain` | 0.6 | 배율 / 0.1~1.0 | 저속에서 곡률 오차를 증폭하는 openpilot LOW_SPEED_Y 곡선에 곱하는 배율이다. `1.0`이 원본. **35 km/h 미만에서만 적용된다**(그 위는 1.0 고정). 20 km/h 아래에서는 이 항이 제어 오차의 95%를 만들어(2026-09-18 실측) 평범한 플랜·센서 노이즈가 요청 토크를 프레임의 17.8%에서 포화시킨다. 35 km/h 위는 포화 0.1%, 리플도 절반이라 손대지 않는다. 경계를 넘는 일은 분당 1회 남짓이고 그때 토크 단차는 중앙값 5카운트라 `steer_delta_up`이 흡수한다. openpilot의 현대차는 저속에서 조향하지 않아 원본 곡선이 이 대역에서 튜닝된 적이 없다(우리는 MDPS 버스 CLU11 속도를 위조해 저속 조향을 연다). 낮추면 저속 핸들 떨림이 줄지만 저속 차선 추종도 같이 느려진다 — 저속 토크의 88%는 리플이 아니라 추종 성분이다. 기본값 `0.6`은 최신 openpilot과 맞춘 값이다: upstream은 2025-10-22(PR #36364)에 저속 계수를 없애고 속도별 비례 이득(`KP_INTERP` 250→0.8)으로 옮겼는데, 그 저속 이득이 우리 `0.6`과 오차 3% 안에서 같다. 우리 `torque_kp_raw=20`은 upstream 단위로 2.0이고 그쪽 공용값은 0.8이라, 전 속도에서 맞추려면 `torque_kp_raw`를 8로 내려야 한다 — 고속은 포화 0%로 문제가 없어 손대지 않았다. 실차 확인 전 값이다. |
